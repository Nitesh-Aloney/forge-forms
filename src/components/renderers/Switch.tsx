...
import type { TSwitchField } from '@/schemas/form-entites/form-fields';
import type { BaseRendererProps } from '@/types';
import { constructPropertyPath } from '@/utils';
import FieldTitle from './FieldTitle';
import OptionsLoader from './OptionsLoader';

interface TSwitchProps extends BaseRendererProps {
  schema: TSwitchField;
}
 ...
 *
 * Renders a group of switch toggles for binary options.
 */
const SwitchComponent: FC<TSwitchProps> = ({ schema, path }) => {
                ...
                optionItems?.map(option => {
                  ...
                  return (
                    <FormControlLabel
                      control={
                        <Switch
                          checked={!!field.value?.[valueStr]}
                          disabled={field.disabled || loading || disable}
                          id={`${field.name}-${option.value}`}
                          name={field.name}
                          onChange={({ target }) => {
                            const newValue = { ...(field.value || {}) };
                            newValue[valueStr] = target.checked;
